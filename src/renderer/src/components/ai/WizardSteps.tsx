...
const LAYOUTS = [
  'title', 'default', 'center', 'section', 'two-col',
  'two-col-wide-left', 'two-col-wide-right', 'three-col',
  'top-bottom', 'big-number', 'quote',
] as const
      ...
      {outline.map((item, i) => (
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <select
              value={LAYOUTS.includes(item.layout as (typeof LAYOUTS)[number]) ? item.layout : 'default'}
              disabled={disabled}
              onChange={(e) => {
                const next = [...outline]
                next[i] = { ...next[i], layout: e.target.value }
                onChange(next)
              }}
              aria-label={`Slide ${i + 1} layout`}
              className="text-[11px] bg-gray-950 text-gray-400 rounded border border-gray-700 px-1.5 py-1 focus:outline-none focus:border-indigo-500 disabled:opacity-30"
            >
              {LAYOUTS.map((l) => (
